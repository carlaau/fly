  ...
  var gather = {
    ...
    ,newmsg: function(){
      if(layui.cache.user.uid !== -1){
        gather.json(layui.cache.user.msgurl, {//'/message/nums/'
          _: new Date().getTime()
        }, function(res){
          if(res.status === 0 && res.count > 0){
            ...
            msg.on('click', function(){
              gather.json(layui.cache.user.msgread, {}, function(res){//'/message/read'
                if(res.status === 0){
                  location.href = layui.cache.user.msghome;//'/user/message/';
                }
              });
            });
          }
    ...
    gather.json(action, data.field, function(res){
      var end = function(){
        if(res.action){
          ...
          gather.form[button.attr('key')||action](data.field, data.form);
        }
      };
      if(res.status == 0){
